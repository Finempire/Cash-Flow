    ...
    return (
        <div className="space-y-4">
            ...
            <div className="md:hidden space-y-3">
                {purchases.length === 0 && (
                    <div className="card p-8 text-center text-xs text-gray-400">No purchases found</div>
                )}
                {purchases.map((p) => (
                    <div key={p.id} className="card">
                        <div className="card-body space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                                <div><span className="text-gray-400">Vendor</span><p className="font-medium">{p.vendor.name}</p></div>
                                <div><span className="text-gray-400">Buyer</span><p className="font-medium">{p.request.buyer.name}</p></div>
                                <div><span className="text-gray-400">Order</span><p className="font-medium">{p.request.order.order_reference}</p></div>
                                <div>
                                    <span className="text-gray-400">Amount</span>
                                    <p className="font-semibold tabular-nums">{formatCurrency(p.invoice_amount)}</p>
                                </div>
                            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                        ...
                        <tbody>
                            {purchases.map((p) => (
                                <tr key={p.id} className="border-b border-gray-100 even:bg-gray-50 hover:bg-blue-50/50">
                                    <td className="px-3 py-2">
                                        <Link href={`/dashboard/runner/purchases/${p.id}`} className="text-blue-600 hover:underline font-medium">
                                            {p.purchase_no}
                                        </Link>
                                    </td>
                                        ...
                                        {p.vendorConfirmation ? (
                                            <span className={
                                                p.vendorConfirmation.status === 'VENDOR_CONFIRMED' ? 'badge-green' :
                                                    p.vendorConfirmation.status === 'SHOWN_TO_VENDOR' ? 'badge-blue' : 'badge-gray'
                                            }>
                            ...
                            {purchases.length === 0 && (
                                <tr>
                                    <td colSpan={9} className="px-3 py-8 text-center text-gray-400">No purchases found</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
